...
class CULaneDataset(Dataset):
    def __init__(self, root_dir, split="train", img_size=(256, 512), transform=None):
        """
        root_dir: path to processed folder (which contains train/ and test/)
        split: "train" or "test"
        img_size: (height, width) to resize to
        ...
        self.image_files = []
        # Walk recursively to collect images and ensure corresponding mask exists
        for dirpath, _, filenames in os.walk(self.img_dir):
            for fname in filenames:
                if fname.lower().endswith(".jpg"):
                    ...
                    mask_rel_path = rel_path.replace(".jpg", "_mask.png")
    ...
    def __getitem__(self, idx):
        ...
        # Load image (BGR) then convert to RGB
        ...
        # Ensure mask is binary [0,1] with shape (1, H, W)
        if mask.ndim == 2:  # (H, W)
            mask = (mask > 0.5).float().unsqueeze(0)
        else:
            # sometimes albumentations returns already with a channel
            mask = (mask > 0.5).float()
